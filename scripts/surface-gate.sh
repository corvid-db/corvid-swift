#!/usr/bin/env bash
# surface-gate.sh — the binding-surface manifest gate (ported from
# corvid-jvm; same rules, this repo's pin source).
#
...
# binding deliberately does not expose it.
#
...
# ---- locate the engine pin (.engine-pin holds it) ---------------------------
pin="$(tr -d '[:space:]' < "$ROOT/.engine-pin")"
case "$pin" in
    v[0-9]*) ;;
    *) echo "surface-gate: cannot read the engine pin from .engine-pin (got '$pin')" >&2; exit 1 ;;
esac
...
# class column must match the engine row verbatim.
join -t "$(printf '\t')" \
